...
setwd("~/Dropbox/RoL/pitcher-16s/Final analysis with Florida/")

library(picante); library(phyloseq)
ps <- readRDS("~/Dropbox/RoL/pitcher-16s/Final analysis with Florida/phyloseq-with-all-sites.RDS")
ps = ps %>% transform_sample_counts(fun = function(x) x/sum(x))
...
saveRDS(weighted.bNTI, "~/Dropbox/RoL/pitcher-16s/Final analysis with Florida/beta-NTI-relative-abundance-Stegen-code.RDS")
