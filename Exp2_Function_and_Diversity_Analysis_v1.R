...
day57_columns <- grep("DAY1_", colnames(asv16s.r))
df_day57 <- as.data.frame(asv16s.r[, day57_columns])
df_day57 <- df_day57[rowSums(df_day57) > 0,] 
df_day57t<- as.data.frame(t(df_day57))
df_day57t$ID <- rownames(df_day57t)
meta57ph3 <- subset(meta.r, ph == "3")
meta57ph3 <- subset(meta57ph3, day == "1")
meta57ph3 <- subset(meta57ph3, temperature == "37")

meta57ph3$ID <- rownames(meta57ph3)
meta57ph3id <- meta57ph3[,15:16]

df_day57tmerge <- left_join(meta57ph3id,df_day57t, by="ID" )
df_day57tmerge <- df_day57tmerge[,-1]
rownames(df_day57tmerge) <- df_day57tmerge$ID
df_day57tmerge <- df_day57tmerge[,-1]
df_day57tmerge <- as.data.frame(df_day57tmerge)
df_day57tmerge <- df_day57tmerge[,colSums(df_day57tmerge) > 0] 

correlation_matrix <- rcorr(as.matrix(otu.gao), type = "spearman")

# Extract the correlation coefficients and p-values
cor_matrix <- correlation_matrix$r
pval_matrix <- correlation_matrix$P

# Filter for significant correlations
significance_level <- 0.05
significant_cor_matrix <- cor_matrix * (pval_matrix < significance_level)
significant_cor_matrix[is.na(significant_cor_matrix)] <- 0

# Convert the correlation matrix to a graph object
...
qgraph(cor_matrix, layout = "spring", 
       ...
       theme = "colorblind")
#blue positive correlations, red negative correlations

library(circlize)
cor_matrix_sparse <- as.data.frame(cor_matrix, sparse = TRUE)
chordDiagram(cor_matrix_sparse, 
             transparency = 0.5,   # Adjust transparency
             annotationTrack = "grid") 
###At family level
tax <- data.frame(tax_table(Exp2.physeq3))
samp <- data.frame(sample_data(Exp2.physeq3))
otu <- data.frame(otu_table(Exp2.physeq3))
...
day57_columns <- grep("DAY1_", colnames(otu.gao))
df_day57 <- as.data.frame(otu.gao[, day57_columns])
df_day57 <- df_day57[rowSums(df_day57) > 0,] 
df_day57t<- as.data.frame(t(df_day57))
df_day57t$ID <- rownames(df_day57t)
meta57ph3 <- subset(samp, ph == "5.6")
meta57ph3 <- subset(meta57ph3, day == "1")
meta57ph3 <- subset(meta57ph3, temperature == "22")
...
correlation_matrix <- rcorr(as.matrix(df_day57tmerge), type = "spearman")
...
##INTERACTION PLOTS
chit.pred2 <- ggpredict(mchit, terms = c("ph", "temperature"))
plot(chit.pred2,  line.size=2, dot.size=4, dodge=1) +
  scale_color_manual(values=c("#1f5776", "#c83126"))




chit.pred <- ggpredict(mchit, terms = c("food", "temperature", "ph"))
chit.pred$facet <- factor(chit.pred$facet, levels = c("3", "4", "5.6"))
plot(chit.pred, facets = TRUE, line.size=2, dot.size=4, dodge=1) +
  scale_color_manual(values=c("#1f5776", "#c83126"))
...
plot(prot.pred, facets = TRUE, line.size=2, dot.size=4, dodge=1, shape=)  +
  scale_color_manual(values=c("#1f5776", "#c83126"))
...
posteriorchit<- posteriorchit[1:12,]
...
ggplot(posteriorresp, aes(x = parameter,
                          shape=nonzero)) +
  geom_hline(yintercept = 0, linetype = 3, 
             size=1, color = "#b0b5b3") +
  geom_pointrange(aes(ymin = ll, ymax = hh, y = m),
  ...
  coord_flip() +
  xlab(NULL) +
  ylab("Estimated effect on Respiration")+
  theme(text = element_text(color = "black", size = 16 ))+
  theme(panel.background = element_rect(fill = "white", colour = "black"))+
  theme(legend.position="none")
...
#### PREDICTIVE MODELS ####
#prep data
meta_all_plant <- subset(meta, container =="pitcher")
#relevel the predictor variables to baselines
meta_all_plant$food <- as.factor(meta_all_plant$food)
meta_all_plant$temperature <- as.factor(meta_all_plant$temperature)
meta_all_plant$ph <- as.factor(meta_all_plant$ph)
meta_all_plant$food <- relevel(meta_all_plant$food, ref = "3")
meta_all_plant$temperature <- relevel(meta_all_plant$temperature, ref = "22")
meta_all_plant$ph <- relevel(meta_all_plant$ph, ref = "5.6")

meta_all_plant_chit <- meta_all_plant[!is.na(meta_all_plant$chitinase), ]
meta_all_plant_prot <- meta_all_plant[!is.na(meta_all_plant$protease), ]
meta_all_plant_resp <- meta_all_plant[!is.na(meta_all_plant$respiration_co2_ppm_hr), ]

meta_all_plant_prot$treatment_combo <- as.factor(meta_all_plant_prot$treatment_combo)

####summarize raw data in pitchers
#chitinase
meta_all_plant$ph <- factor(meta_all_plant$ph, levels=c("3", "4", "5.6"))

a <-ggplot(meta_all_plant, aes(x=ph, y=chitinase, color=ph)) + geom_boxplot(outlier.shape=NA) + 
  geom_jitter(alpha=.5)+ theme_classic()+scale_color_manual(values=c("#CA64A3","#9A4EAE", "#301934"))+theme(legend.position="none")
b <-ggplot(meta_all_plant, aes(x=temperature, y=chitinase, color=temperature)) + geom_boxplot(outlier.shape=NA) + 
  geom_jitter(alpha=.5)+ theme_classic()+scale_color_manual(values=c("#1f5776", "#c83126"))+theme(legend.position="none")
c <-ggplot(meta_all_plant, aes(x=food, y=chitinase, color=food)) + geom_boxplot(outlier.shape=NA) + 
  geom_jitter(alpha=.5)+ theme_classic()+scale_color_manual(values=c("#90ee90", "#228822"))+theme(legend.position="none")
ggarrange(b, c, a, nrow=1)


ggplot(meta_all_plant, aes(x = food, y = chitinase, shape = food, color = temperature)) + 
  geom_boxplot(aes(group = interaction(food, temperature)), outlier.shape = NA) +
  geom_jitter(aes(group = interaction(food, temperature)), 
              position = position_jitterdodge(dodge.width = 0.75, jitter.width = 0.4), size=3, alpha=.7) + 
  scale_color_manual(values = c("#1f5776", "#c83126")) +
  facet_wrap(~ ph) + theme_bw() +
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()) +
  theme(legend.position="none")

#protease
a <-ggplot(meta_all_plant, aes(x=ph, y=protease, color=ph)) + geom_boxplot(outlier.shape=NA) + 
  geom_jitter(alpha=.5)+ theme_classic()+scale_color_manual(values=c("#CA64A3","#9A4EAE", "#301934"))+theme(legend.position="none")
b <-ggplot(meta_all_plant, aes(x=temperature, y=protease, color=temperature)) + geom_boxplot(outlier.shape=NA) + 
  geom_jitter(alpha=.5)+ theme_classic()+scale_color_manual(values=c("#1f5776", "#c83126"))+theme(legend.position="none")
c <-ggplot(meta_all_plant, aes(x=food, y=protease, color=food)) + geom_boxplot(outlier.shape=NA) + 
  geom_jitter(alpha=.5)+ theme_classic()+scale_color_manual(values=c("#90ee90", "#228822"))+theme(legend.position="none")
ggarrange(b, c, a, nrow=1)


ggplot(meta_all_plant, aes(x = food, y = protease, shape = food, color = temperature)) + 
  geom_boxplot(aes(group = interaction(food, temperature)), outlier.shape = NA) +
  geom_jitter(aes(group = interaction(food, temperature)), 
              position = position_jitterdodge(dodge.width = 0.75, jitter.width = 0.4), size=3, alpha=.7) + 
  scale_color_manual(values = c("#1f5776", "#c83126")) +
...
#### matching up ancombc sig asv to those whose genomes we have to look for important functions ####
lb_genome_df <- read.csv("Assays_57strains_Aug2018Metadata.csv", header=TRUE)
ancombc_sig <- all_res_sig
ancombc_sig <- ancombc_sig %>%
  mutate(ASV_ID = taxon)

ancombc_withlb <- left_join(ancombc_sig, lb_genome_df, by="ASV_ID")

ancombc_withlb_subset <- ancombc_withlb[,c(37:55)]

ancombc_withlb_subset <- ancombc_withlb_subset[!is.na(ancombc_withlb_subset$Strain_name), ]
unique(ancombc_withlb_subset$ASV_ID) #12


#[1] "d007cee006889a1090c2d5bfb35a1953" "20b3df08cd5fac2be26a2928155bab69" "f36c50279a8fa1c121cb9dda1ab1cb42"
#[4] "d32e579b3ae7b2aae8d5bf9f027c29af" "16da9716b4d7c3c9c904f3f799d67dfe" "17f5a48986ee11c3fc2b99d8e5ff4b1c"
#[7] "7ead17a076e7b7b419bf50abe7eee3f4" "2743823e7602e70cfa467627e69d2774" "3cd9ce63f826137d18ca1a11155a21ad"
#[10] "38c27ceaed634984c1225a82648cf571" "4d72007c70f4abc0bf9e8fe8b826d07d" "63afe8e6aac58bf0d670a82ca5bc574c"


ancombc_withlb_subset_newnames <- left_join(ancombc_withlb_subset, newasv, by=c("ASV_ID"= "ASV"))


ancombc_withlb_subset_newnames_unique <- ancombc_withlb_subset_newnames %>%
  distinct(ASV_ID, .keep_all = TRUE)
#could only find one, for my ASV7, their ASV27 was acidovorax, but not xylophilus


xylo <- read.delim("M10D5C16.tsv", sep = "\t", header = TRUE)

xylo_clean <- xylo[,c(6,13:15)]

detach("package:plyr", unload=TRUE)

xylo_clean2 <- xylo_clean %>%
  group_by(Name) %>%
  summarise(count = n())
